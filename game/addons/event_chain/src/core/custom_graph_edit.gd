...
# 处理所有和编辑器交互，撤销重做等。插件的特定逻辑只发生在子类EventChainGraphTemplate中
...
func _init() -> void:
	_setup_gui()
	EventChainGraphDataType.setup_valid_connection_types(self)
...
func is_node_connected_to_output(node: GraphNode, idx: int) -> bool:
	var name = node.get_name()
	for c in get_connection_list():
		if c["from"] == name and c["from_port"] == idx:
			return true
	return false
...
func _disconnect_output(node: GraphNode, idx: int) -> void:
	var name = node.get_name()
	for c in get_connection_list():
		if c["from"] == name and c["from_port"] == idx:
			disconnect_node(c["from"], c["from_port"], c["to"], c["to_port"])
